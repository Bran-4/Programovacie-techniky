...
void buildHeapSiftUp(int data[], const size_t length)
{
    for (size_t i = 1; i < length; i++) {
        // Použijeme siftUp na každý prvok
        siftUp(data, i);
    }
}
...
void siftDown(int data[], const size_t topIndex, const size_t length)
{
    ...
    if(ileft>=length){//konec ked neexistuje lavy
        return;
    }
    //lavy existuje
    ...
    //existuje pravy potomok a ma hodnotu viac ako data[iswap]
    if(iright<length && data[iright]<data[iswap]){
        iswap=iright;
    }
    //je potrebne urobit swap
    if(iswap!=topIndex){
        swap(&data[topIndex],&data[iswap]);
        siftDown(data,iswap,length);//rekurzia
    }
...
int main() {
    int data[11]={7, 2, 1, 2, 8, 5, 3, 4, 2, 2, 6};  // tu mozete doplnit testovaci kod
