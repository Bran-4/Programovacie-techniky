...
queue<int> reverse(const list<int>& data) noexcept {
        // Vytvoríme novú prázdnu frontu
        std::queue<int> result;

        // Používame reverzný iterátor na prechod zoznamu odzadu dopredu
        for (auto it = data.rbegin(); it != data.rend(); it++) {
            result.push(*it); // Pridávame prvky do fronty
        }
        return result; // Vraciame novovytvorenú frontu// tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    vector<vector<int>> velkyvektor;
    for(int i=0;i<size1;i++){
        std::vector<int> malyVector(size2, value);
        velkyvektor.push_back(malyVector);
    }
    return velkyvektor; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
string connectWithSpace(const string & text, const int number) noexcept {
    string cislo = std::to_string(number);
    string spoj;
    spoj=text+" "+cislo;
    return spoj; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int sum(istringstream & text) noexcept {
    ...
    return sucet; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for (const auto& word : sentence) {
        ...
        if (it != translator.end()) {
            ...
            result.push_back("?");  // Ak preklad neexistuje, pridáme "?"
        }
    }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    for (int i = 0; i < mathExpression.length(); ++i) {
        if(mathExpression[i] == '<' || mathExpression[i] == '(' || mathExpression[i] == '[' || mathExpression[i] == '{'){
            zatvorky.push(bracketMap[mathExpression[i]]);
        }
        else if(mathExpression[i] == '>' || mathExpression[i] == ')' || mathExpression[i] == ']' || mathExpression[i] == '}'){
            if(zatvorky.empty()){return false;}
            if(zatvorky.top()!=mathExpression[i]){
                return false;
            }
            zatvorky.pop();
        }
...
list<int> createList(const list<int>::size_type length, const list<int>::size_type indexOfOne) {
    ...
    advance(it, indexOfOne);  // Presunieme sa na index indexOfOne
    *it = 1;  // Nastavíme hodnotu na 1
...
int main() {
    //prva
    std::list<int> result={1,2,4};
    std::queue<int> result2;
    result2=reverse(result);
    //druha
    std::queue<int> result3;
    result3.push(3);
    result3.push(7);
    result3.push(5);
    reverse(result3);
    //tretia
    vector<vector<int>> skuska;
    skuska= create2dVector(2,5,10);
    //stvrta
    string veta;
    veta= connectWithSpace("pes",20);
    //pat
    istringstream vet("20 as 20");
    int ha=sum(vet);
    //sest
    ...
    list<string> sentence = {"I", "rea", "a", "book"};

    list<string> translated = translate(translator, sentence);
//sedem
    bool je_ci_ne= areParenthesisPaired("(!))");
//osem
    int a=execute(&maxValue, 20, 50);
    //desat
    list<int> nice;
    try{
        nice= createList(10,9);
    }
