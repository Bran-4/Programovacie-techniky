...
public:
    ...
    String(const char * slovo){
        if(slovo){
            data=new char[strlen(slovo)+1];
            strcpy(data,slovo);
        }
        else{
            data=new char[1];
    ...
    String(const String& slovo){
        data=new char[strlen(slovo.data)+1];
        strcpy(data,slovo.data);
    }
    ...
    void append(const char *text){
        if(text){
            char *data2;
            data2=new char[strlen(data)+ strlen(text)+1];
            strcpy(data2,data);
            strcpy(data2+strlen(data),text);
            delete[] data;
            data=data2;
        }
    }
