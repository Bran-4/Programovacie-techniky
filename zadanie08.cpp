...
double average(const initializer_list<int> & data) {
    if (data.size() == 0) {
        // Vyhodenie výnimky, ak je inicializačný zoznam prázdny
        throw std::invalid_argument("inicializacny zoznam je prazdny");
    }

    // Výpočet sumy a následne priemeru pomocou std::accumulate
...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    ...
    return std::find(data.begin(),data.end(),value); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int>::const_iterator findInRange200to400(const list<int> & data) noexcept {
    return std::find_if(data.begin(),data.end(), isInRange200to400); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
void helpAfterExam2(list<int> & points) noexcept {
    for(int & index:points){
        index=incrementPointsIfItHelps(index);
...
int main() {
    std::list<int> myList = {1, 2, 3, 1};
    //2uloha
    list<int>::const_iterator pes= findValue(myList,1);
    std::cout<<*pes;
    //uloha3
//uloha 5
    incrementAll(myList);
    //uloha 6
    std::list<int> myList2={ 100, 90, 91, 92, 93, 53, 54, 55, 56, 57, 91, 92 };
    //helpAfterExam1(myList2);
    //uloha7
    helpAfterExam2(myList2);
    //uloha
    std::vector<int> pess={1,2,3,55,1};
    sort2(pess);

    std::list<int> myList3 = {1, 2, 3, 1};
    std::list<int> myList4 = {4, 6, 1, 0};
    std::list<int> myList5;
    myList5=add(myList3,myList4);
